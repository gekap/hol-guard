...
SEED_ROOT=${SEED_ROOT:-/tmp/batch1-native-seed}
...
A candidate is accepted only when the installed live PreToolUse path is native,
no Python module parses, transports, renders, recovers, or evaluates the live
decision, and real process tracing observes no Python executable.
...
for entry in "${candidates[@]}"; do
  ...
  sha=${entry##*:}
  safe=${branch//\//-}
  log_dir="/tmp/rust-pretool-direct-$safe"
...
The candidate passed the direct-native source gate, process-execution trace,
compiled real-binary adversarial integration, Rust workspace checks, and all
discovered native PreToolUse integration suites. No Python executable or
Python live-decision bridge was accepted.
EOF
    git add -A
    exit 0
  fi
...
No historical candidate satisfied the literal direct-native requirement. The
migration must not fall back to a Python transport path.
EOF
git add -A
exit 1
