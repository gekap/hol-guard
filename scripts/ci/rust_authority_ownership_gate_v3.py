...
def imports_python_evaluator(path: Path) -> bool:
    ...
    return any(
        ...
        and (node.module or "").endswith("command_evaluation")
        and any(alias.name == "evaluate_command" for alias in node.names)
...
def retired_native_mode_gate() -> None:
    runtime = read(Path("src/codex_plugin_scanner/guard/native_runtime.py"))
    command = read(Path("src/codex_plugin_scanner/guard/native_command_model.py"))
    patterns = (
        r"NativeMode[^\n]*strict",
        r"HOL_GUARD_NATIVE[^\n]*strict",
        r"mode\s*(?:==|in)\s*[^\n]*[\"']strict[\"']",
    ...
    found = [pattern for pattern in patterns if re.search(pattern, runtime + "\n" + command, re.IGNORECASE)]
    if found:
        raise RuntimeError(f"retired strict native mode remains: {found}")
