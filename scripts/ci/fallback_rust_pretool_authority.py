#!/usr/bin/env python3
"""Apply a conservative Rust-only PreToolUse authority when no historical candidate passes."""
...
def patch_guard_command() -> None:
    ...
    source = path.read_text(encoding="utf-8")
    if "pub struct PreToolDecisionV1" in source:
        return
    marker = "\n#[cfg(test)]\nmod tests {\n"
...
fn normalized_sensitive_command(value: &str) -> bool {
...
fn destructive_command(value: &str) -> bool {
    ...
    (lowered.contains("rm -rf") || lowered.contains("rm -fr"))
        && [" /", " -- /", " $home", " ~"].iter().any(|needle| lowered.contains(needle))
}
...
fn exact_safe_command(model: &CanonicalCommandV1) -> bool {
    ...
    model.segments.iter().all(|segment| {
        ...
        match basename {
            ...
            "rg" | "grep" => !normalized_sensitive_command(&segment.text),
            _ => false,
        }
    })
}
...
pub fn evaluate_pre_tool(request: &CommandModelRequestV1) -> Result<PreToolDecisionV1, String> {
    ...
    if normalized_sensitive_command(normalized) && exfiltration_command(normalized) {
        return Ok(pretool_decision(
            model,
            "block",
    ...
    if normalized_sensitive_command(normalized) {
        return Ok(pretool_decision(
            model,
            "review",
...
def patch_runtime() -> None:
    ...
    enum_old = "    CommandModel(CommandModelRequestV1),\n    Health(Value),\n"
    enum_new = "    CommandModel(CommandModelRequestV1),\n    PreToolUse(CommandModelRequestV1),\n    Health(Value),\n"
    if enum_old in source:
    ...
    if '"pre-tool-command-authority-v1".into()' not in source:
        ...
        source = source.replace(
            feature_anchor,
            feature_anchor + '        "pre-tool-command-authority-v1".into(),\n',
            1,
        )
...
fn evaluate_pre_tool_bytes(bytes: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let response = evaluate_pre_tool(&request)?;
...
'''
    if "fn evaluate_pre_tool_bytes(" not in source:
        ...
        source = source.replace(eval_anchor, pretool + eval_anchor, 1)
    resident_old = '''        ResidentRequestV1::Operation(ResidentOperationV1::CommandModel(request)) => {
            evaluate_command_model_request(&request)
        }
        ResidentRequestV1::Operation(ResidentOperationV1::Health(_request)) => {
'''
    resident_new = '''        ResidentRequestV1::Operation(ResidentOperationV1::CommandModel(request)) => {
            evaluate_command_model_request(&request)
        }
...
'''
    ...
    run_old = '''        [command, flag] if command == "command-model" && flag == "--stdin" => {
            let bytes = read_stdin_bounded()?;
            let response = evaluate_command_model_bytes(&bytes)?;
            write_bytes_response(&response)
        }
'''
    run_new = run_old + '''        [command, flag] if command == "pre-tool" && flag == "--stdin" => {
            let bytes = read_stdin_bounded()?;
            let response = evaluate_pre_tool_bytes(&bytes)?;
            write_bytes_response(&response)
        }
'''
    if run_old in source and 'command == "pre-tool"' not in source:
...
def replace_native_command_bridge() -> None:
    path = Path("src/codex_plugin_scanner/guard/native_command_model.py")
    path.write_text('''"""Transport-only bridge to the Rust PreToolUse command authority.

This module does not parse, classify, evaluate, or lower command policy in
Python. It validates the request-bound native response and returns it to the
harness boundary. Native failure is represented as ``None`` and callers must
fail closed.
"""
...
def patch_hook_worker() -> None:
    ...
    source = path.read_text(encoding="utf-8")
    import_anchor = "from ..native_runtime import native_mode, review_post_tool_native\n"
    if "review_pre_tool_native" not in source:
        source = source.replace(
            import_anchor,
            "from ..native_command_model import review_pre_tool_native\n" + import_anchor,
            1,
        )
    event_old = '''        event_name = self._hook_event_name(payload)
...
'''
    event_new = '''        event_name = self._hook_event_name(payload)
        ...
        request = self._request_from_payload(
'''
    if event_old in source:
        source = source.replace(event_old, event_new, 1)
    elif "event_name == \"PreToolUse\"" not in source:
        raise RuntimeError("HookWorker event dispatch marker not found")
...
'''
    if "def _pre_tool_command(" not in source:
        if helper_anchor not in source:
            raise RuntimeError("HookWorker helper anchor not found")
